...
def get_available_gpus():
    ...
    try:
        ...
        available_gpus = [i for i, mem in enumerate(gpu_memory_used) if mem <= 15]
        ## remove gpu 7 from the list of available gpus
        # available_gpus = [gpu for gpu in available_gpus if gpu != 7]
...
def run_expts():
    ...
    cleaning_approaches = ['mmcl', 'mmcl_ssl']
    # lrs = [1e-5, 4e-5, 8e-5, 1e-4, 4e-4, 8e-4, 1e-3, 4e-3]      ## 8 lrs
    lrs = np.linspace(1e-4, 4e-4, 7)[1:-1]      ## 5 lrs
    ## make sure lrs are rounded to 8 decimal places
    lrs = [round(lr, 8) for lr in lrs]
    print(lrs)
